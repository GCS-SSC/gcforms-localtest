...
let gcNotifyConnector: NotifyConnector | null = null;

async function getGcNotifyConnector(): Promise<NotifyConnector> {
  if (process.env.LOCAL_SKIP_NOTIFY === "true") {
    throw new Error("GC Notify is disabled in local mode.");
  }

  if (gcNotifyConnector === null) {
    const { GCNotifyConnector } = await import("@gcforms/connectors");
    gcNotifyConnector = await GCNotifyConnector.defaultUsingApiKeyFromAwsSecret(
...
export default async (
  ...
  try {
    ...
    const connector = await getGcNotifyConnector();
