...
export async function verifyAccessToken(accessToken: string, formId: string) {
  if (process.env.LOCAL_AUTH_BYPASS === "true") {
    const localToken = getLocalVerifiedAccessToken(accessToken, formId);
    validateIntrospectedToken(localToken, formId);
    return localToken;
  }
...
function getLocalVerifiedAccessToken(
  accessToken: string,
  formId: string,
): VerifiedAccessToken {
  const parts = accessToken.split(":");
  const expirationEpochTime =
    Math.floor(Date.now() / 1000) + CACHE_EXPIRY_DELAY_IN_SECONDS;

  if (parts[0] === "local" && parts.length >= 3 && parts[1] && parts[2]) {
    return {
      expirationEpochTime,
      serviceAccountId: parts[2],
      serviceUserId: parts[1],
    };
  }
  ...
  if (parts[0] === "local" && parts.length === 2 && parts[1]) {
    return {
      ...
      serviceUserId: formId,
    };
  }

  return {
      expirationEpochTime: Math.floor(Date.now() / 1000) + CACHE_EXPIRY_DELAY_IN_SECONDS,
...
async function generateIntrospectedAccessToken(
  ...
  const { introspectAccessToken } = await import("@lib/integration/zitadelConnector.js");
